...
class ListaDeTricipesScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Consumer<TricipesManager>(
          builder: (_, tricipesManager, __) {
            if (tricipesManager.search.isEmpty) {
              return const Text('Exercicios De Trícipes');
            } else {
                  ...
                  return GestureDetector(
                    onTap: () async {
                      final search = await showDialog<String>(
                          context: context,
                          builder: (_) => SearchDialog(tricipesManager.search));
                      if (search != null) {
                        tricipesManager.search = search;
                      }
                    },
                    child: Container(
                        width: constraints.biggest.width,
                        child: Text(
                          tricipesManager.search,
                ...
                return IconButton(
                  ...
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => CriarExercicio(3)),
                    );
                  },
                );
              } else {
      ...
      body: Consumer<TricipesManager>(
        builder: (_, tricipesManager, __) {
          final filteredProducts = tricipesManager.filteredProducts;
          if (tricipesManager.filteredProducts.isEmpty) {
            return EmptyCard(
